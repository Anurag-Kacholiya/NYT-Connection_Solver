...
def process_game(words):
    ...
    # For reasoning, we look at the UN-double-centered, unweighted raw similarity matrices
    # Because double centering introduces negative numbers that make percentage breakdowns confusing.
    # Actually, we can just use the absolute sum of the centered matrices to show driving forces.
    ...
    for group_indices in best_partition:
        ...
        scores = {"Semantic": 0.0, "Lexical": 0.0, "WordNet": 0.0, "WikiData": 0.0}
        pairs = 0
        for i in range(4):
            for j in range(i+1, 4):
                ...
                # A feature only "contributes" if it pulls them together (>0).
