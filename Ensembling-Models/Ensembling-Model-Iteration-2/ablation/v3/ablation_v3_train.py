...
# Split boundaries
# ─────────────────────────────────────────────────────────────────────────────
TEST_SIZE = 100   # last 100 games → test
VAL_SIZE  = 137   # last 137 games before test → val
# TRAIN_END and VAL_END are computed dynamically from dataset size in _train_impl()
...
def _train_impl(csv_path: str, out_dir: str = "models"):
    ...
    n           = len(all_games)
    VAL_END     = n - TEST_SIZE          # test  = [VAL_END, n)
    TRAIN_END   = VAL_END - VAL_SIZE     # val   = [TRAIN_END, VAL_END)
                                         # train = [0, TRAIN_END)
