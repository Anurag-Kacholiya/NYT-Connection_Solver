...
def build_matrices(embedder, words,
    ...
    Flags:
        ...
        use_lexical           → include character n-gram lexical similarity
        use_knowledge_graphs  → include WikiData, Datamuse, ConceptNet, Wikipedia Categories
    """
    ...
    if use_lexical:
        lex_sim     = double_center(embedder.get_lexical_similarity(words))
        phonetic_sim = double_center(embedder.get_phonetic_similarity(words))
        morpho_sim  = double_center(embedder.get_morphological_similarity(words))
        named.append(("Lexical_sim",       lex_sim))
        named.append(("Phonetic_sim",      phonetic_sim))
        named.append(("Morphological_sim", morpho_sim))
    ...
    if use_knowledge_graphs:
        wikidata_sim   = double_center(embedder.get_wikidata_similarity(words))
        datamuse_sim   = double_center(embedder.get_datamuse_similarity(words))
        conceptnet_sim = double_center(embedder.get_conceptnet_similarity(words))
        wikipedia_sim  = double_center(embedder.get_wikipedia_categories_similarity(words))
        grid_prop_uniq = double_center(embedder.get_grid_property_uniqueness_matrix(words))
        grid_cat_uniq  = double_center(embedder.get_grid_category_uniqueness_matrix(words))
        grid_dm_uniq   = double_center(embedder.get_grid_datamuse_uniqueness_matrix(words))
        named.append(("WikiData_sim",          wikidata_sim))
        named.append(("Datamuse_sim",           datamuse_sim))
        named.append(("ConceptNet_sim",         conceptnet_sim))
        named.append(("WikipediaCategories_sim", wikipedia_sim))
        named.append(("GridPropertyUniqueness_sim", grid_prop_uniq))
        named.append(("GridCategoryUniqueness_sim", grid_cat_uniq))
        named.append(("GridDatamuseUniqueness_sim", grid_dm_uniq))
...
_ALL_MATRICES = [
    ...
    "Datamuse_sim",
    "ConceptNet_sim",
    "WikipediaCategories_sim",
    "GridPropertyUniqueness_sim",
    "GridCategoryUniqueness_sim",
    "GridDatamuseUniqueness_sim",
]
...
# ── Weighted matrix fusion helper  (Path A) ──────────────────────────────────

def compute_per_matrix_separation(S, matrices, remaining_items):
    """
    Compute one feature per matrix for weighted matrix fusion (Path A).

    Feature = separation = mean_in - mean_out for the candidate group S
    in each matrix. This is the signal the Logistic Regression uses to
    learn per-matrix weights.

    Args:
        S               : list of 4 word indices
        matrices        : list of (16×16) double-centred similarity matrices
        remaining_items : set of word indices still in play

    Returns:
        np.ndarray of shape (len(matrices),), dtype float32
    """
    S_set     = set(S)
    outsiders = [i for i in remaining_items if i not in S_set]
    feats = []
    ...
    for mat in matrices:
        ...
        if outsiders:
            out_sims = [mat[item][other] for item in S for other in outsiders]
            mean_out = float(np.mean(out_sims))
        else:
            mean_out = 0.0

        feats.append(mean_in - mean_out)   # separation
...
# Uncertainty filter threshold  (Path A)
# std([rf, gbm, ranker, lr]) above this → models disagree → don't commit greedily
# Set to 0.25 to account for natural calibration differences across the 4 models
...
def simulate_game_ml(words, matrices, matrix_names, clf, gt_partitions_list,
                     clf_ranker=None, clf_lr_fusion=None, use_candidates=True):
    """
    ...
    while remaining_items and lives > 0:
        ...
        # ── Path A: Weighted Matrix Fusion (LR on per-matrix separations) ─
        if clf_lr_fusion is not None:
            per_mat_seps = np.array(
                [compute_per_matrix_separation(list(S), matrices, remaining_items)
                 for S in subsets],
                dtype=np.float32
            )
            probs_lr = clf_lr_fusion.predict_proba(per_mat_seps)[:, 1]
            lr_min, lr_max = probs_lr.min(), probs_lr.max()
            if lr_max > lr_min:
                probs_lr = (probs_lr - lr_min) / (lr_max - lr_min)
        else:
            probs_lr = None

        # ── Combine: Path A (20%) + Path B RF/GBM/Ranker (80%) ───────────
        if probs_ranker is not None and probs_lr is not None:
            base_probs = 0.15*probs_rf + 0.15*probs_gbm + 0.50*probs_ranker + 0.20*probs_lr
        elif probs_ranker is not None:
            base_probs = 0.20 * probs_rf + 0.20 * probs_gbm + 0.60 * probs_ranker
        elif probs_lr is not None:
            base_probs = 0.35 * probs_rf + 0.35 * probs_gbm + 0.30 * probs_lr
        else:
            base_probs = (probs_rf + probs_gbm) / 2.0

        # ── Uncertainty: std across all available model probs ────────────
        model_probs_list = [probs_rf, probs_gbm]
        if probs_ranker is not None:
            model_probs_list.append(probs_ranker)
        if probs_lr is not None:
            model_probs_list.append(probs_lr)
        # shape (n_models, n_candidates) → std along model axis → (n_candidates,)
        uncertainty = np.std(np.stack(model_probs_list, axis=0), axis=0)
        ...
        top_uncertainty = float(uncertainty[top_idx])

        if len(remaining_items) == 16 and not guessed and top_base_score < CONFIDENCE_THRESHOLD:
            # Filter score_lookup to top 250 candidates for solver efficiency
            top_subsets = sorted(subsets, key=lambda s: score_lookup.get(s, 0.0), reverse=True)[:250]
            solver_score_lookup = {s: score_lookup[s] for s in top_subsets}
            best_partition, solver_score = solve_ml(solver_score_lookup, remaining_items)
            if best_partition:
        ...
        # Build index map for uncertainty lookup
        subset_to_idx = {s: i for i, s in enumerate(subsets)}

        # First pass: skip candidates where models disagree (high uncertainty)
        made_guess = False
        for subset, prob in scored:
            if subset in guessed:
                continue
            idx = subset_to_idx.get(subset, -1)
            if idx >= 0 and uncertainty[idx] > UNCERTAINTY_THRESHOLD:
                continue   # models disagree — don't commit greedily
            made_guess = True
            guessed.add(subset)
        ...
        # Fallback: if every remaining candidate was uncertain, take the best-scoring one
        if not made_guess:
            for subset, prob in scored:
                if subset in guessed:
                    continue
...
def evaluate_games(csv_path: str, limit: int = None, ablation: bool = False):
    try:
        _evaluate_impl(csv_path, limit, ablation)
    except WikidataForbiddenError as e:
        print(f"\nCRITICAL ERROR: {e}")
        print("Stopping run to avoid further Wikidata API block. Run again later.")
        sys.exit(1)

def _evaluate_impl(csv_path: str, limit: int = None, ablation: bool = False):
    print("Loading data...")
