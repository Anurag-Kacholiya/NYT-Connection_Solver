...
def evaluate_games(csv_path: str, limit: int = None):
    ...
    for idx, game in enumerate(tqdm(games, desc="Solving Games")):
        ...
        # 0.8 / 0.2 empirically balances semantic meaning with character-level tricks
        ...
        print("Predictions sequence:")
        for p in pred_partitions:
            if p in gt_partitions:
                status = "Correct"
            else:
                is_one_away = any(len(p.intersection(gt)) == 3 for gt in gt_partitions)
